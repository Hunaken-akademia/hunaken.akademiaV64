...
function cloudStateFrom(row) {
  return {
    ...
    betRecords: normalizeSavedArray(row?.bet_records, []),
  };
}

function buildCloudState({ cart, payoutOddsInput, betRecords }) {
  return {
    cart: normalizeSavedArray(cart, []),
    payoutOddsInput: normalizeSavedString(payoutOddsInput, ""),
    betRecords: normalizeSavedArray(betRecords, []),
  };
}

async function loadCloudTicketState(userId, accessToken) {
  if (!CLOUD_SAVE_ENABLED || !userId || !accessToken) return null;
  const query = `user_id=eq.${encodeURIComponent(userId)}&select=cart,payout_odds_input,bet_records,updated_at&limit=1`;
...
function mergeBetRecords(cloudRecords, localRecords) {
  const out = [];
  const seen = new Set();
  for (const r of [...normalizeSavedArray(cloudRecords, []), ...normalizeSavedArray(localRecords, [])]) {
...
export default function App() {
  ...
  // Googleログイン＋クラウド保存（保存対象は買い目・配当・舟券収支のみ）
    ...
    const loadCloud = async () => {
      ...
      try {
        ...
        if (row) {
          ...
          const nextCart = cloud.cart.length ? cloud.cart : cart;
          const nextPayout = cloud.payoutOddsInput !== "" ? cloud.payoutOddsInput : payoutOddsInput;
          const nextBetRecords = mergeBetRecords(cloud.betRecords, betRecords);
          setCart(nextCart);
          setPayoutOddsInput(nextPayout);
          setBetRecords(nextBetRecords);
          try { localStorage.setItem("hunaken_betRecords", JSON.stringify(nextBetRecords)); } catch (_) { /* noop */ }
          saveDeviceAutoSave({ cart: nextCart, payoutOddsInput: nextPayout });
          const state = buildCloudState({ cart: nextCart, payoutOddsInput: nextPayout, betRecords: nextBetRecords });
          lastCloudSaveJsonRef.current = JSON.stringify(state);
          setCloudMsg("✓ クラウド保存から復元しました");
        } else {
          const state = buildCloudState({ cart, payoutOddsInput, betRecords });
          await saveCloudTicketState(cloudAuth.user.id, cloudAuth.session.access_token, state);
          lastCloudSaveJsonRef.current = JSON.stringify(state);
          setCloudMsg("✓ クラウド保存を開始しました");
        }
      } catch (_) {
        setCloudMsg("クラウド保存の読み込みに失敗しました。端末内保存で継続します。");
      } finally {
        if (!cancelled) {
          setCloudLoaded(true);
  ...
  }, [cloudLoaded, cloudAuth.user?.id, cloudAuth.session?.access_token, cart, payoutOddsInput, betRecords]);
  ...
  const signOutCloud = async () => {
    ...
    setCloudLoaded(true);
    setCloudMsg("Googleログアウトしました。以後は端末内保存のみです。");
  };
  ...
  const saveCloudNow = async () => {
    ...
    try {
      ...
      await saveCloudTicketState(cloudAuth.user.id, cloudAuth.session.access_token, state);
      lastCloudSaveJsonRef.current = JSON.stringify(state);
      setCloudMsg("✓ 今の買い目・配当・舟券収支をクラウド保存しました");
    } catch (_) {
      setCloudMsg("クラウド保存に失敗しました。通信状況を確認してください。");
    }
  };
      ...
      const parseSection = (sec) => {
        ...
        const row = (labels) => {
          for (const label of labels) {
            ...
            if (i === -1) continue;
            const vals = readPercentValues(sec.slice(i + label.length), 6);
            if (vals) return vals;
          }
          return null;
        };
  ...
  }, [autoSaveReady, cart, payoutOddsInput]);
  ...
  const clearBetHistoryOnly = async () => {
    if (betRecords.length === 0) {
      setBetMsg("削除する舟券履歴がありません");
      return;
    }
    if (!window.confirm("舟券の収支履歴をすべて削除します。買い目リストと配当入力は残ります。よろしいですか？")) return;
    await persistBets([]);
    setBetMsg("✓ 舟券の収支履歴を削除しました（買い目リスト・配当入力は維持）");
  };
        ...
        <div style={{ marginTop: 28, borderTop: "1px solid #1d3149", paddingTop: 18 }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
            <button
              ...
              disabled={betRecords.length === 0}
              style={{
                padding: "8px 12px", borderRadius: 8, cursor: betRecords.length ? "pointer" : "not-allowed",
                background: betRecords.length ? "#3a1f1f" : "#16273c",
                color: betRecords.length ? "#ffb3a8" : "#5e7a92",
