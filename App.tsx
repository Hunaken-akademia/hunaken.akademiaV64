...
function cloudStateFrom(row) {
  return {
    ...
    records: normalizeSavedArray(row?.records, []),
  };
}

function buildCloudState({ cart, payoutOddsInput, betRecords, records }) {
  return {
    ...
    records: normalizeSavedArray(records, []),
  };
}

async function loadCloudTicketState(userId, accessToken) {
  if (!CLOUD_SAVE_ENABLED || !userId || !accessToken) return null;
  const query = `user_id=eq.${encodeURIComponent(userId)}&select=cart,payout_odds_input,bet_records,records,updated_at&limit=1`;
...
export default function App() {
    ...
    const loadCloud = async () => {
      ...
      try {
        ...
        if (row) {
          ...
          const state = buildCloudState({ cart: nextCart, payoutOddsInput: nextPayout, betRecords: nextBetRecords, records: nextRecords });
          ...
          // ログイン中は端末内保存に書かない（クラウドが正・端末内保存は未ログイン専用）
          const state = buildCloudState({ cart: emptyCart, payoutOddsInput: emptyPayout, betRecords: emptyBetRecords, records: emptyRecords });
  ...
  useEffect(() => {
    ...
    const state = buildCloudState({ cart, payoutOddsInput, betRecords, records });
  ...
  }, [cloudLoaded, cloudAuth.user?.id, cloudAuth.session?.access_token, cart, payoutOddsInput, betRecords, records]);
  ...
  const saveCloudNow = async () => {
    ...
    try {
      ...
      await saveCloudTicketState(cloudAuth.user.id, cloudAuth.session.access_token, state);
      lastCloudSaveJsonRef.current = JSON.stringify(state);
      setCloudMsg("✓ 今の買い目・配当・舟券収支・予想記録をクラウド保存しました");
    } catch (_) {
      setCloudMsg("クラウド保存に失敗しました。通信状況を確認してください。");
    }
  };
